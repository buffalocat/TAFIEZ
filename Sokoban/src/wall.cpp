...
void Wall::draw(GraphicsManager* gfx) {
	Point3 p = pos_;
	gfx->cube.push_instance(glm::vec3(p), glm::vec3(1.0f), BlockTexture::Wall, gfx->wall_color(p.z));
	if (modifier_) {
		modifier()->draw(gfx, p);
	}
}
...
void ArtWall::draw(GraphicsManager* gfx) {
	...
	model->push_instance(glm::vec3(p), glm::vec3(1.0f), BlockTexture::Blank, gfx->wall_color(p.z));
	if (modifier_) {
		modifier()->draw(gfx, p);
	}
}
