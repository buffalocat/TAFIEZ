...
void LoadMenuState::generate_callbacks() {
	backup_save_.callback = [this]() { savefile_->load_subsave_dispatch(SaveType::Emergency, 0, rps_); };
	for (int i = 0; i < auto_saves_.size(); ++i) {
		auto_saves_[i].callback = [this, i]() { savefile_->load_subsave_dispatch(SaveType::Auto, i, rps_); };
	}
	for (int i = 0; i < manual_saves_.size(); ++i) {
		manual_saves_[i].callback = [this, i]() { savefile_->load_subsave_dispatch(SaveType::Manual, i, rps_); };
	}
}
...
const float TOP_ROW = 0.82f;
const float VERT_SPACE = -0.38f;
